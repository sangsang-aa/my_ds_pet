...
> 上 / 下两个方向是**完整的一上一下**（起跳 → 悬停 → 降落），首尾都是落地姿势，正向
> 循环本身即平滑，因此**按正常循环播放**（不再来回倒放）；左 / 右同理。
>
> 竖向走动在**时长到 / 撞边缘**后不会立刻切回待机，而是**继续播到末帧（落地姿势）**
> 再回待机——否则宠物会在半空中、或在"起跳前的预备姿势"上突然变回站立，看起来像瞬移
> 或"没落地"。
...
- 窗口大小自动适配第一帧的像素尺寸，并按 `kScaleFactor = 0.70` 缩小显示
  （源图 384x384 → 屏幕上约 269x269）；嫌大/嫌小改 `src/core/config.h` 里的常量即可
...
| `src/core/` | **基础功能**：`config.h` 常量 · `gdi.h` · `util.*`（路径/报错/随机）· `assets.*`（帧加载）· `render.*`（分层窗口绘制） |
| `src/pet/` | **桌宠本体**：`pet.*`（动画池 + 待机/行走/点击/拖拽/害羞状态机）· `walk_logic.h`（纯逻辑） |
| `src/bubble/` | **发表情包**：`bubble.*`（气泡窗口，独立定时器 + 点击穿透）· `bubble_logic.h`（纯逻辑） |
